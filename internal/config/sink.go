...
// defSinkConfig is the default sink configuration.
var defSinkConfig = []SinkConfig{
	{
		Name:        "stdout",
		Type:        types.StdOut,
		SourcePorts: true,
...
func DecodeSinkConfigMap(cfg map[string]any) ([]SinkConfig, error) {
	out := make([]SinkConfig, 0, len(cfg))
	for name, params := range cfg {
		sc := SinkConfig{Name: name}
		...
		if err != nil {
			return nil, fmt.Errorf("creating sink config decoder: %w", err)
		}

		if err := d.Decode(params); err != nil {
			return nil, fmt.Errorf("decoding sink configuration for '%s': %w", name, err)
		}
...
func stringToSinkTypeHookFunc() mapstructure.DecodeHookFunc {
	return func(f, t reflect.Type, data any) (any, error) {
		...
		if t != reflect.TypeFor[types.SinkType]() {
			return data, nil
		}
